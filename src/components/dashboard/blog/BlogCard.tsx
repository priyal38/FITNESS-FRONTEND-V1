...
import { BlogData } from '../../../pages/user/blog/Blog';
...
const BlogCard = ({ data} :Props) => {
  ...
  <Link to={`${data._id}`}>
        <div className="h-full  bg-surface-200 rounded-lg overflow-hidden shadow-inner shadow-slate-400">
          <img className="lg:h-48 md:h-48 h-44 w-full object-cover object-center" src={`http://localhost:5000/${data.coverImg}`} alt="blog"/>
