...
const SelectedWorkoutTable = ({ selectedDate, tabledata, getTableData , onDateChange }: Props) => {

    const axiosPrivate = useAxiosPrivate();
    const [modalOpen, setModalOpen] = useState<boolean>(false);
    const [checkedItems, setCheckedItems] = useState<{ [date: string]: boolean[] }>({});

    // useEffect(() => {
    //     // Reset checkedItems when tabledata changes
    //     setCheckedItems(prevCheckedItems => {
    //         const newCheckedItems = { ...prevCheckedItems };
    //         newCheckedItems[selectedDate] = newCheckedItems[selectedDate] || Array(tabledata.length).fill(false);
    ...
    const handleCheckboxChange = async (id: string, type: string ,index: number) => {
        try {
         
                await axiosPrivate.put(`/progress/incrementCompletedCount/${type}/${id}`);
               
                // setCheckedItems(prevCheckedItems => {
                //     const newCheckedItems = { ...prevCheckedItems };
                //     newCheckedItems[selectedDate] = [...(newCheckedItems[selectedDate] || Array(tabledata.length).fill(false))];
                //     newCheckedItems[selectedDate][index] = true;
                //     return newCheckedItems;
                // });
                getTableData(selectedDate);
            }
                                ...
                                tabledata.map((tableItem, key) => (
                                    <tr key={key} className=" border-b bg-surface-200 border-gray-700 hover:bg-gray-600">
                                        <td scope="row" className="px-6 py-4  ">
                                            {tableItem.workoutId ? tableItem.workoutId.title : tableItem.title}
                                        </td>
                                        ...
                                        <td className="  py-4 px-6 ">
                                            <div className="flex items-center justify-center">
                                                <input type="checkbox" className="w-6 h-6 text-primary-500  rounded focus:ring-primary-500 ring-offset-gray-800 focus:ring-offset-gray-800 focus:ring-1 bg-gray-700 border-gray-600"  onChange={() => handleCheckboxChange(tableItem._id, tableItem.workoutType,key )}
                                                 checked={checkedItems[selectedDate]?.[key] || false}
                                                 disabled={checkedItems[selectedDate]?.[key] || false}
                                                />
